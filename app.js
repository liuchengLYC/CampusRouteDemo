...
// ================================
// 10. Flatten API information into array
// ================================
...
function formatInstruction(step, legNumber, stepNumber) {
  ...
  switch (type) {
    ...
    case "turn": {
      ...
      actionText = rawRoadName ? `${turnText}進入${roadNameText}` : turnText;
      break;
    }
...
// ================================
// 13. Auto step switching
// ================================
function tryAutoAdvanceStep(userLat, userLng) {
  if (navigationSteps.length === 0) return;
  ...
  const [endLat, endLng] = currentStep.endLocation;
  const distanceToStepEnd = getDistanceInMeters(userLat, userLng, endLat, endLng);

  if (distanceToStepEnd <= AUTO_STEP_SWITCH_DISTANCE) {
    currentStepIndex += 1;
    renderSteps();
    updateCurrentStepCard();
    updateStepButtons();
